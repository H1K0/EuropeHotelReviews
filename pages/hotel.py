...
@callback(
    [Output('hotel-address', 'value'),
     Output('crossfilter-natio', 'options'),
...
def update_page(hotel,nationalities,translate_on):
    ...
    figure = px.scatter(
        ...
        size_max=60,
        labels={
            "Review_Month": "Месяц отзывов",
            "Reviewer_Score": "Средняя оценка",
            "Review_Weighted_Score": "Кол-во отзывов",
    ...
    return hotel_data["Hotel_Address"][0],[{'label': i, 'value': i} for i in sorted(df[(df["Hotel_Name"]==hotel)]['Reviewer_Nationality'].unique())],avg_tot,diff,pos_rev,neg_rev,figure
